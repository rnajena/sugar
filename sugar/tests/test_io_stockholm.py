...
def test_stk():
    ...
    seqs = read(fname)
    assert seqs.meta.stk.GF.ID == 'UPSK'
    assert seqs.meta.stk.GC.SS_cons == '.AAA....<<<<aaa....>>>>'
    assert 'virus RNA' in seqs.meta.stk.GF.RT
...
def test_stk2():
    ...
    seqs = read(fname)
    assert seqs.todict()['O83071/192-246'].meta.stk.GR.SA.startswith('999')
    with tempfilename() as fn2:
        seqs.write(fn2, 'stockholm')
        ignore=['#=GF CC']
        assert normalize_content(fname) != normalize_content(fn2)
